...
public class UAP_AccessibilityManager : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (UnityEngine.InputSystem.Keyboard.current != null && UnityEngine.InputSystem.Keyboard.current.leftShiftKey.isPressed && UnityEngine.InputSystem.Keyboard.current.aKey.isPressed && (UnityEngine.InputSystem.Keyboard.current.leftWindowsKey.isPressed || UnityEngine.InputSystem.Keyboard.current.leftCommandKey.isPressed))
		{
			ToggleAccessibility();
		}
		//////////////////////////////////////////////////////////////////////////
	...
	private int GetTouchCount()
	{
		...
		// NEW Input System - Emulación de dedos con mouse
		var mouse = Mouse.current;
		var keyboard = Keyboard.current;

		if (mouse != null && mouse.leftButton.isPressed &&
			(keyboard != null && (keyboard.leftAltKey.isPressed || keyboard.leftCommandKey.isPressed)))
		{
			return 2;  // 2 dedos (Left Click + Left Alt/Command)
		}

		if (mouse != null && mouse.rightButton.isPressed &&
			(keyboard != null && (keyboard.leftAltKey.isPressed || keyboard.leftCommandKey.isPressed)))
		{
			return 3;  // 3 dedos (Right Click + Left Alt/Command)
		}

		if (mouse != null && mouse.leftButton.isPressed)
			return 1;  // 1 dedo (Left Click)
	...
	static public void RecalculateUIElementsOrder(GameObject parent = null)
	{
		...
		{
			AccessibleUIGroupRoot[] allContainers = FindObjectsByType<AccessibleUIGroupRoot>(FindObjectsInactive.Exclude, FindObjectsSortMode.None) as AccessibleUIGroupRoot[];
			foreach (AccessibleUIGroupRoot container in allContainers)
				container.RefreshContainer();
		}
	}
